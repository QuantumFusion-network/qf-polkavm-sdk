...
async function main() {
    ...
    const contractAddress: string = await new Promise((resolve, reject) => {
        uploadExtrinsic.signAndSend(ALICE, ({ events = [], status }) => {
    ...
    // const userData = '001';
    const userDataRaw = new Uint8Array(2048);
    const view = new DataView(userDataRaw.buffer);
    const n = 42;
    ...
    const execute = async () => {
        ...
        const executionData: Codec[] = await new Promise((resolve, reject) => {
            executeExtrinsic.signAndSend(ALICE, ({ events = [], status }) => {
                events.forEach(({ event: { data, method, section } }) => {
